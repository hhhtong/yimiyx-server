...
class WXBizDataCrypt {
  ...
  decryptData(_encryptedData: string, _iv: string) {
    // base64 decode
    const sessionKey: Buffer = Buffer.from(this.sessionKey, 'base64')
    const encryptedData: Buffer = Buffer.from(_encryptedData, 'base64')
    const iv: Buffer = Buffer.from(_iv, 'base64')
    let decoded: any
